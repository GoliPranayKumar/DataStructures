...
export function AdminDashboard() {
  ...
  const [pending, setPending] = useState<Array<{id:string; name:string|null; email:string|null; college:string|null; submittedAt:string; documents:string[];}>>([]);

  const refreshPending = async () => {
    try {
      const list = await supabaseApi.getPendingOrganizerVerifications();
      setPending(list.map(i => ({ id: i.id, name: i.name, email: i.email, college: i.college, submittedAt: i.submittedAt, documents: i.documents })));
    } catch {
      setPending([]);
    }
  };
  ...
  const handleApproveOrganizer = async (id: string) => {
    ...
    await supabaseApi.reviewOrganizerVerification(id, 'approved');
  ...
  const handleRejectOrganizer = async (id: string) => {
    ...
    if (!reason) return;
    await supabaseApi.reviewOrganizerVerification(id, 'rejected', reason);
    await refreshPending();
